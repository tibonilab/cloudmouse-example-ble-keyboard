...
namespace CloudMouse::Hardware
{
    ...
    void DisplayManager::update()
    {
        ...
        if (needsRedraw && currentScreen == Screen::VOLUME_FEEDBACK)
        {
    ...
    void DisplayManager::renderVolumeFeedback(const String &message)
    {
        if (!sprite)
        {
            Serial.println("❌ Cannot render - sprite not initialized");
        ...
        // Draw header
        drawHeader("CloudMouse Volume");

        // Show volume message - BIG and CENTERED
        sprite->setTextSize(4);
        drawCenteredText(message, 160, COLOR_ACCENT);
